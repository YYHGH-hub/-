    ...
    def func(self, x):
        ...
        if un == 1 and dn > 0:
            ...
            if keys in dict1.keys():
                ...
                mouse.position=(int(position[0]), int(position[1]))
                mouse.click(Button.left, 1)
                mouse.position=(position1[0], position1[1])
            keynames = []
            keytype = []
        if dn == 0 and un > 0:
            keynames = []
    ...
    def add(self):
        try:
            ...
            while key == "n" or key == "N":
                ...
                mouse = pynput.mouse.Controller()
                position = mouse.position #记录快捷键的鼠标位置
            key = "n"
            name_key = ""
            while key == "n" or key == "N":
                name_key = input("输入快捷键名称和功能键（英文字符），空格隔开如（”橡皮擦 f8“或”+“连接的组合键如”橡皮擦 ctrl+shift+!“）:").split(" ")#记录快捷键的名字
